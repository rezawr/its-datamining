...
textclassifier = make_pipeline(
    ...
    SGDClassifier(loss='hinge', penalty='l2', learning_rate='optimal', random_state=12)
)
...
model = SGDClassifier(loss='hinge', penalty='l2', learning_rate='optimal', random_state=12)
...
def main():
    ...
    results_df.to_csv('results__svc.csv', index=False)
    performance_df.to_csv('performance__svc.csv', index=False)
    totalThread_df.to_csv('thread_monitoring__svc.csv', index=False)
